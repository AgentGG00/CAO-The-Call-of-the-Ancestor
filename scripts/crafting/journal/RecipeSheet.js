...
export class MastercraftedRecipeSheet extends foundry.applications.sheets.journal.JournalEntryPageProseMirrorSheet {
    ...
    static DEFAULT_OPTIONS = {
        actions: {},
        classes: ["cao-the-call-of-the-ancestors.mastercrafted"],
        includeTOC: true,
        position: {
            height: "auto",
        },
    ...
    static CLEAN_ARRAYS = ["flags.cao-the-call-of-the-ancestors.modifierList"];
    ...
    async _preparePartContext(partId, context, options) {
        ...
        context.inventoryActors = {
            "useCraftingActor": game.i18n.localize("cao-the-call-of-the-ancestors.recipeApp.useCraftingActor"),
            ...context.actors,
        }
    ...
    _setupEventListeners(html, isView, rerender) {
        ...
        html.querySelectorAll(".mastercrafted-ingredient:has(.empty-ingredient), button[data-action='add-component']").forEach((el) => {
            el.addEventListener("click", async (ev) => {
                ev.stopPropagation();
                const ingredientId = el.closest(".mastercrafted-ingredient").dataset.ingredientId || "";
                const { ingredient, component } = await this.recipe.addComponent(ingredientId, "", game.i18n.localize("cao-the-call-of-the-ancestors.recipeApp.component"), "icons/commodities/materials/powder-grey.webp", []);
                if (!component) return;
                new ComponentEditForm(component, this.document).render(true);
            });
        });
